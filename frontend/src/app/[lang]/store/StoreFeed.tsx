 ...
 *
 * It still opens on real pieces: the server renders an unpersonalised first
 * page and hands it down as `initial`, so a crawler is given a shelf rather
 * than an empty document, and a person sees pieces instead of a skeleton. The
 * personalised feed takes over from page 1 and the existing de-duplication
 * absorbs anything that appears in both.
 *
 ...
 */
export function StoreFeed({ lang, initial }: { lang: Lang; initial: Piece[] }) {
  ...
  // `?q=` makes a search shareable, and it is what the site's structured data
  // promises Google when it offers the shelf's search under the result. Read
  // from the URL directly rather than through `useSearchParams`, which would
  // force this route out of static rendering for one optional parameter.
  useEffect(() => {
  ...
  return (
    <div className="page pt-6">
      ...
      {pieces.length === 0 && !failed && initial.length === 0 ? (
        <div className="mt-6 grid grid-cols-2 gap-3" aria-hidden>
